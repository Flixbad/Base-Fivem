...
local LiveAFK = require 'modules.liveafk.client'
local currentWeapon
...
function client.openInventory(inv, data)
    ...
    left.weight = PlayerData.weight or left.weight
    left.type = left.type or 'player'
    left.label = left.label or 'Poches'
    left.maxWeight = left.maxWeight or shared.playerweight
...
local function updateInventory(data, weight)
	...
	SendNUIMessage({
		...
		data = {
			items = data,
			itemCount = itemCount,
			weight = weight or PlayerData.weight,
		}
	})
...
local acardiaPayPromise = nil

RegisterNUICallback('acardiaPayResult', function(result, cb)
	print(('[ACARDIA PAY] NUI callback received: method=%s cardId=%s pin=%s'):format(
		tostring(result and result.method), tostring(result and result.cardId), tostring(result and result.pin and '****' or 'nil')
	))
	cb('ok')
	if acardiaPayPromise then
		acardiaPayPromise:resolve(result)
		acardiaPayPromise = nil
	end
end)

RegisterNUICallback('buyItem', function(data, cb)
	print('[ACARDIA PAY] buyItem callback triggered')
	local payMethod = 'cash'
	local cardId = nil
	local cardPin = nil
	local hasBankResource = GetResourceState('acardia_bank') == 'started'

	if hasBankResource and (not data.currency or data.currency == 'money') then
		print('[ACARDIA PAY] Fetching cards...')
		local cards = lib.callback.await('acardia_bank:atmCards', false) or {}
		print(('[ACARDIA PAY] Got %d cards'):format(#cards))
		if #cards > 0 then
			acardiaPayPromise = promise.new()
			SendNUIMessage({ action = 'acardiaPayOpen', cards = cards })
			print('[ACARDIA PAY] Waiting for NUI result...')
			local result = Citizen.Await(acardiaPayPromise)
			print(('[ACARDIA PAY] Got result: method=%s cardId=%s'):format(
				tostring(result and result.method), tostring(result and result.cardId)
			))
			if not result or result.method == 'cancel' then
				print('[ACARDIA PAY] Cancelled')
				cb(false)
				return
			end
			payMethod = result.method or 'cash'
			cardId = result.cardId and tonumber(result.cardId) or nil
			cardPin = result.pin
		end
	end

	print(('[ACARDIA PAY] Sending to server: method=%s cardId=%s'):format(payMethod, tostring(cardId)))
