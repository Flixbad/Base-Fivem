...
local function canAffordItem(inv, currency, price, paymentMethod, data)
	if paymentMethod == 'card' and currency == 'money' then
		local hasBankRes = GetResourceState('acardia_bank') == 'started'
		if not hasBankRes then
			return { type = 'error', description = 'Système bancaire indisponible' }
		end
		print(('[ACARDIA PAY SRV] verifyCardPayment cardId=%s pin=%s price=%s owner=%s'):format(
			tostring(data.cardId), tostring(data.cardPin and '****' or 'nil'), tostring(price), tostring(inv.owner)
		))
		local result = exports.acardia_bank:verifyCardPayment(data.cardId, data.cardPin, price, inv.owner)
		print(('[ACARDIA PAY SRV] result=%s'):format(json.encode(result or 'nil')))
		if not result or not result.ok then
			return { type = 'error', description = (result and result.msg) or 'Paiement refusé' }
		end
		return true, true
	end
...
local function removeCurrency(inv, currency, price, paymentMethod, cardId)
	if paymentMethod == 'card' and currency == 'money' then
		return
	end
	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local canAfford, alreadyDebited = canAffordItem(playerInv, currency, price, paymentMethod, data)
				...
				if not alreadyDebited then
					removeCurrency(playerInv, currency, price, paymentMethod, data.cardId)
				end
